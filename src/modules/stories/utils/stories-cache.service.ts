...
/**
 * StoriesCacheService — lightweight Redis wrapper for the stories surface (Phase 7
 * Plan 02). Mirrors CoursesCacheService verbatim — read-through getOrSet + SCAN/UNLINK
 * pattern invalidation, tolerant of Redis errors (API stays up if Redis flaps).
 *
 * Namespace: geonline-admin:stories:* (Plan 01 STORIES_INVALIDATE_PATTERN constant).
 *
 * Currently used for invalidation only — list/detail caching is OFF in Plan 02
 ...
 */
@Injectable()
export class StoriesCacheService {
    private readonly logger = new Logger(StoriesCacheService.name);

    public static readonly DEFAULT_TTL_SECONDS = 300;

    constructor(@InjectRedis() private readonly redis: Redis) {}

    public async getOrSet<T>(key: string, fn: () => Promise<T>, ttlSeconds: number = StoriesCacheService.DEFAULT_TTL_SECONDS): Promise<T> {
        try {
