 ...
 * 403-not-404 distinction (CONTEXT D-19 + ROADMAP §"Phase 5" SC #4):
 *   - admin           → 200 (sees all)
 *   - teacher (own)   → 200
 *   - teacher (other) → 403 'courses.forbidden_scope'
 *   - curator         → 403 'courses.forbidden_scope' (default-deny per WEBINAR_SCOPE_RULES.curator)
 *
 ...
 * the ForbiddenException branch in service code because WEBINAR_SCOPE_RULES.curator
