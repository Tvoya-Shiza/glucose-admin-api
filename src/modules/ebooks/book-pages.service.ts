...
export class BookPagesService {
    ...
    public async replacePages(bookId: number, dto: ReplacePagesDto) {
        ...
        const pageCount = await this.prisma.$transaction(
            async (tx) => {
                for (const group of chunk(pages, BookPagesService.UPSERT_CHUNK)) {
        ...
        // Index the changed pages (best-effort). Replace semantics → the request body is
        // authoritative, so we hand the content straight to the indexer (no MySQL re-read).
        const indexInputs: IndexPageInput[] = pages.map((p) => ({ page_number: p.page_number, text_content: p.text_content ?? null }));
